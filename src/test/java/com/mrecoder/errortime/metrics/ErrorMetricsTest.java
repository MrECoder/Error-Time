...
class ErrorMetricsTest {
    ...
    void appErrorsAreCountedSeparatelyPerErrorCodeAndStatus() {
        errorMetrics.recordAppError("VALIDATION_ERROR", 400);
        errorMetrics.recordAppError("VALIDATION_ERROR", 400);
        errorMetrics.recordAppError("RESOURCE_NOT_FOUND", 404);
    ...
    void downstreamErrorsAreTaggedBySource() {
        errorMetrics.recordDownstreamError("feign:downstream-service", "INTERNAL_SERVICE_ERROR");
        errorMetrics.recordDownstreamError("amqp:error-time.events", "VALIDATION_ERROR");
